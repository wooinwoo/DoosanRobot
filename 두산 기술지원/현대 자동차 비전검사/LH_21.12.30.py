...
def signal_reset(): 
    ...
    set_digital_outputs(bit_start=1, bit_end=16, val=0b000000000000000)
    drl_report_line(1)
def vision():
    wait(1)#로봇 떨림 대기시간
...
while(True):
    ...
    for step in range(1,15):
        ...
        """ 로봇 상호 인터록
        if step==1: value=17 #RH 본넷 작업 중
        if 1<setp<7: value=18 #RH 루프 작업 중
        if step==8: value=19 #RH 뒷판넬 작업 중
        """
        ...
        while(True):
            if step==1: set_output_register_bit(17,ON) # LH 본넷 작업 중
            if setp==1:
        ...
        """ 로봇 상호 인터록
        if step==1: 
            set_output_register_bit(17,ON) # LH 본넷 작업 중
            set_output_register_bit(18,OFF)
            set_output_register_bit(19,OFF)
        if 1<setp<7: 
            set_output_register_bit(18,ON) # LH 루프 작업 중
            set_output_register_bit(17,OFF)
            set_output_register_bit(19,OFF)
        if step==8: 
            set_output_register_bit(19,ON) # LH 뒷 판넬 작업 중
            set_output_register_bit(17,OFF)
            set_output_register_bit(18,OFF)
        """
        
        set_output_register_bit(6,ON)#로봇 작업 중 전달
        if step==1: #front_4400_0
        ...
        elif step==3: #top_2586_300
            movej(Global_sub3_p1)
            vision()
            #movej(home_P)
        elif step==4: #top_1928_300
        ...
        elif step==7: #opposite side_1210_0
